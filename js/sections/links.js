RENDERERS.links = async function(container){
  ...
  container.appendChild(sectionBlock('🧠 Tab 1 (demo) — PyTorch CNN Pavement Defect Model', 'Illustrative content from the priezent Result Engine page — not a real trained model on this platform.', el('div',{},[cnnFacts, el('div',{style:'height:14px;'}), cnnTable])));

  // ---- Tab 2: network predictions (demo) ----
  ...
  container.appendChild(sectionBlock('🌍 Tab 2 (demo) — 21,000 km Network Predictions', '5 of 1,022 sample link-level predictions shown on the priezent page — illustrative, not a real prediction run on this platform\'s network.geojson.', el('div',{},[netKpis, el('div',{style:'height:14px;'}), netTable])));

  // ---- Tab 3: WIM weighbridges (demo) ----
  ...
  container.appendChild(sectionBlock('⚖️ Tab 3 (demo) — WIM Weighbridge Compliance', 'Illustrative station list from the priezent page — not the real weighbridge_district_coverage.json dataset used elsewhere on this platform.', wimGrid));

  // ---- Tab 4: Nile bridges (demo) ----
  ...
  container.appendChild(sectionBlock('🌉 Tab 4 (demo) — UGNBMS Nile Bridges', 'Illustrative bridge ratings from the priezent page — not the platform\'s real geo/bridges.geojson register (see Bridges & Culverts for that).', el('div',{},[bridgeGrid, el('p',{class:'footnote', style:'margin-top:12px;'},'Overall "Structural Health Index" shown on the source page: 96.2%.')])));

  // ---- Tab 5: solar worlds roster (demo, explicitly fictional) ----
  ...
  container.appendChild(sectionBlock('🪐 Tab 5 (demo, fictional) — "10 Solar Worlds AI Workforce"', 'Explicitly a fictional/creative concept from the priezent project — no such workforce, agents or planetary operation exists. Shown here purely as a demo of the source page\'s content.', worldsTable));

  // ---- Tab 6: satellite / LiDAR (demo) ----
  ...
  container.appendChild(sectionBlock('🛰️ Tab 6 (demo) — Satellite Remote Sensing & LiDAR', 'Illustrative content from the priezent page — this platform does not currently ingest satellite or LiDAR survey data.', satGrid));

  container.appendChild(el('div',{class:'callout warn', style:'margin-top:8px;'},[
    el('strong',{},'Reminder: '),
    'Everything in the six "Tab (demo)" sections above is decorative content from Prisca\'s personal priezent project, reproduced here for reference. For this platform\'s real, sourced figures, see Overview, Network Explorer, Maintenance & Condition, Bridges & Culverts and Analytics & Insights.'
  ]));
};
