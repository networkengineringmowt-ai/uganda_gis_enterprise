RENDERERS.overview = async function(container){
  ...
  // ---- Live verification & enterprise links ----
  ...
  const linkGrid = el('div',{style:'display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:18px;'});
  enterpriseLinks.forEach(l=>{
    linkGrid.appendChild(el('a',{href:l.url, target:'_blank', rel:'noopener noreferrer', class:'card card-pad hoverable', style:`display:block;border-left:4px solid ${l.a};text-decoration:none;color:inherit;`},[
  ...
  container.appendChild(sectionBlock('🚀 Live Verification & Enterprise Links', 'Live, publicly deployed projects — opens in a new tab.', linkGrid));
};
